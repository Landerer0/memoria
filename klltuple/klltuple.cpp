...
uint64_t KLLTuple::saveData(string outputFileName){
    //FORMATO:
    // KLLTuple TRADICIONAL: isMRL, minElement, maxElement, n, epsilon, delta, c, minK, numTotalElementos, 
...
uint64_t KLLTuple::rank(pair<int64_t,int64_t> element){
    uint64_t rank = 0;
...
vector<uint64_t> KLLTuple::rank(vector<pair<int64_t,int64_t>> elements){
    ...
    for(int i=0;i<elements.size();i++){
        ...
        while(actualPair < vectorElementosSize && elements.at(i).first >= vectorElementos.at(actualPair).first.first){ // comparo el num Elementos menores
